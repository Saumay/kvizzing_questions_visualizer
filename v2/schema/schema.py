...
_TOPICS_JSON = HERE.parent / "pipeline" / "config" / "topics.json"
_topic_ids = [t["id"] for t in json.loads(_TOPICS_JSON.read_text())] if _TOPICS_JSON.exists() else [
    "history", "science", "literature", "technology", "sports", "geography",
    "entertainment", "food_drink", "art_culture", "business", "etymology",
    "mythology", "geology", "meme", "politics", "general",
]

TopicCategory = Enum("TopicCategory", {tid: tid for tid in _topic_ids}, type=str)  # type: ignore[misc]
...
class DiscussionRole(str, Enum):
    ...
    elaboration   = "elaboration"   # post-answer context, trivia, or explanation
...
class DiscussionEntry(BaseModel):
    ...
    has_media: bool = Field(
        ...
        description="True if this discussion message included an image/video/audio/document "
                    "attachment in the original chat. Only tracked for hint, answer_reveal, "
                    "and elaboration roles — reaction GIFs and memes in chat/attempt/confirmation "
