...
| `review-finalize --classifications PATH [--min-confidence 0.6]` | Merge an AI-produced classifications JSON into `data/auto_review_suggestions.json` and copy to `static/data/` so the /review UI surfaces ✨ AI-suggested status badges. Auto-archives a timestamped copy of the raw classifications under `data/ai_classifications/` |
| `extract-loop [--batch-size N] [--finalize-only \| --emit-only \| --status]` | File-based async orchestrator for AI-as-LLM bulk extraction. Each call: finalize any batch with `output.json`, then emit a fresh `input.json` for the next N dates. Batches under `v2/data/extract_batches/<batch-id>/`. Used during a multi-month bulk run when the AI reviewer (Claude in-session) acts as the stage 2 LLM |
| `promote-rejected --thread-id ID [--reason TEXT]` | Rescue a curator-confirmed Missed Q from rejected_candidates: emit a focused single-Q input bundle for the AI to extract, then on re-invocation finalize into extraction_output and reimport. Used to back the one-click "rescue" action in the /review UI |
