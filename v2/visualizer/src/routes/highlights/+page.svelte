...
<div class="space-y-6">
  <!-- Header -->
  <div>
    <h1 class="text-2xl font-bold text-gray-900">Highlights</h1>
    <p class="text-sm text-gray-500 mt-1">The group's story, in numbers</p>
  </div>
  ...
  <div class="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit">
    {#each tabs as tab}
      <button
        ...
        class="px-4 py-1.5 text-sm font-medium rounded-lg transition-colors {activeTab === tab.id ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}"
      >
        {tab.label}
      </button>
    {/each}
    ...
    <div class="grid sm:grid-cols-2 gap-6">
      ...
      <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100">
          ...
          <p class="text-xs text-gray-400 mt-0.5">By questions solved</p>
        </div>
        <div class="divide-y divide-gray-50">
          {#each topSolvers as member, i}
            <div class="px-5 py-3 flex items-center gap-3 hover:bg-gray-50 transition-colors">
              <span class="text-sm font-bold text-gray-300 w-6 text-right flex-shrink-0">
                {i + 1}
              </span>
              <MemberAvatar username={member.username} color={member.color} displayName={member.display_name} size="sm" />
              <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-gray-800 truncate">{member.display_name}</p>
                {#if member.avg_solve_time_seconds}
                  <p class="text-xs text-gray-400">avg {formatTime(member.avg_solve_time_seconds)}</p>
                {/if}
              </div>
      ...
      <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100">
          ...
          <p class="text-xs text-gray-400 mt-0.5">By questions posed</p>
        </div>
        <div class="divide-y divide-gray-50">
      ...
      {#if speedLeaderboard.length > 0}
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden sm:col-span-2">
          <div class="px-5 py-4 border-b border-gray-100">
            <h2 class="font-semibold text-gray-900 flex items-center gap-2">
              <span class="text-lg">⚡</span> Speed Leaderboard
            </h2>
            <p class="text-xs text-gray-400 mt-0.5">Average solve time (min. {MIN_SOLVES} solves)</p>
          </div>
          <div class="divide-y divide-gray-50">
            {#each speedLeaderboard as member, i}
              <div class="px-5 py-3 flex items-center gap-3 hover:bg-gray-50 transition-colors">
                ...
                <MemberAvatar username={member.username} color={member.color} displayName={member.display_name} size="sm" />
                <div class="flex-1 min-w-0">
                  <p class="text-sm font-medium text-gray-800">{member.display_name}</p>
                  <p class="text-xs text-gray-400">{member.questions_solved} solves</p>
                </div>
  ...
  {#if activeTab === 'topics'}
    <div class="space-y-5">
      ...
      <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
        <h2 class="font-semibold text-gray-900 mb-4">Difficulty Breakdown</h2>
        <div class="space-y-3">
          ...
          <div class="flex flex-wrap gap-2">
            {#each topics as topic}
              ...
              <a
                ...
                class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-700 hover:border-orange-300 hover:bg-orange-50 transition-colors"
              >
                <span>{topic}</span>
                <span class="text-xs text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded">{count}</span>
              </a>
            {/each}
          </div>
        </div>
      {:else}
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
          ...
          <p class="text-sm text-gray-400">No topics tagged yet.</p>
        </div>
      {/if}
    </div>
  {/if}
    ...
    <div class="grid sm:grid-cols-2 gap-6">
      ...
      <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div class="divide-y divide-gray-50">
          {#each mostDiscussed as q}
            <a href="/question/{q.id}" class="px-5 py-3 flex items-start gap-3 hover:bg-gray-50 transition-colors block">
              <div class="flex-1 min-w-0">
                <p class="text-sm text-gray-700 line-clamp-2">{q.question.text}</p>
                <p class="text-xs text-gray-400 mt-1">{q.question.asker}</p>
              </div>
        ...
        <div class="divide-y divide-gray-50">
          {#each quickest as q}
            <a href="/question/{q.id}" class="px-5 py-3 flex items-start gap-3 hover:bg-gray-50 transition-colors block">
              <div class="flex-1 min-w-0">
                <p class="text-sm text-gray-700 line-clamp-2">{q.question.text}</p>
                <p class="text-xs text-gray-400 mt-1">{q.answer?.solver ?? '—'}</p>
              </div>
      ...
      <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden sm:col-span-2">
        <div class="px-5 py-4 border-b border-gray-100">
          ...
          <p class="text-xs text-gray-400 mt-0.5">Most wrong attempts</p>
        </div>
        <div class="divide-y divide-gray-50">
          {#each trickiest as q}
            <a href="/question/{q.id}" class="px-5 py-3 flex items-start gap-3 hover:bg-gray-50 transition-colors block">
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-2 mt-1">
                  <p class="text-xs text-gray-400">{q.question.asker}</p>
                  {#if q.question.topic}
