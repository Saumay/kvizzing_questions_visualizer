...
<div class="space-y-5 max-w-2xl">
  ...
  <div class="flex items-center gap-3">
    <a href="/" class="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 transition-colors">
  ...
  <div class="flex items-center gap-3 flex-wrap">
    <div class="flex items-center gap-2">
      ...
      <div>
        <p class="text-sm font-semibold text-gray-800">{q.asker}</p>
        <p class="text-xs text-gray-400">{formatDate(question.date)}</p>
      </div>
    </div>
  ...
  <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
    <p class="text-gray-900 text-base leading-relaxed font-medium">{q.text}</p>
    {#if q.has_media}
    ...
    {#if revealed && q.tags && q.tags.length > 0}
      <div class="flex gap-2 mt-3 flex-wrap">
        {#each q.tags as tag}
          <span class="text-xs px-2 py-0.5 bg-gray-100 text-gray-500 rounded-full">{tag}</span>
        {/each}
      </div>
    {/if}
  </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      {#if stats.time_to_answer_seconds}
        <div class="bg-white rounded-xl border border-gray-200 p-3 text-center shadow-sm">
          <p class="text-lg font-bold text-gray-900">{formatTime(stats.time_to_answer_seconds)}</p>
          <p class="text-xs text-gray-400 mt-0.5">Solve time</p>
        </div>
      {/if}
      <div class="bg-white rounded-xl border border-gray-200 p-3 text-center shadow-sm">
        <p class="text-lg font-bold text-gray-900">{stats.wrong_attempts}</p>
        <p class="text-xs text-gray-400 mt-0.5">Wrong attempts</p>
      </div>
      {#if stats.hints_given > 0}
        <div class="bg-white rounded-xl border border-gray-200 p-3 text-center shadow-sm">
          <p class="text-lg font-bold text-gray-900">{stats.hints_given}</p>
          <p class="text-xs text-gray-400 mt-0.5">Hints given</p>
        </div>
      {/if}
      {#if stats.unique_participants > 0}
        <div class="bg-white rounded-xl border border-gray-200 p-3 text-center shadow-sm">
          <p class="text-lg font-bold text-gray-900">{stats.unique_participants}</p>
          <p class="text-xs text-gray-400 mt-0.5">Participants</p>
        </div>
      {/if}
    </div>
  {/if}
  ...
  {#if revealed && a?.solver}
    <div class="flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl px-4 py-3">
      ...
      {#if a.confirmation_text}
        <span class="text-xs text-green-600 ml-auto italic">"{a.confirmation_text}"</span>
      {/if}
    </div>
  {/if}
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      >
        <div class="flex items-center gap-2">
          ...
          <span class="text-sm font-semibold text-gray-700">Discussion</span>
          <span class="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">{question.discussion.length}</span>
        </div>
    ...
    {#if adj.next}
      <a href="/question/{adj.next.id}" class="flex items-center gap-2 text-sm text-gray-500 hover:text-orange-600 transition-colors group max-w-[45%]">
        ...
        <div>
          ...
          <p class="text-xs font-medium text-gray-700 group-hover:text-orange-600 line-clamp-2">{adj.next.question.text.slice(0, 60)}…</p>
        </div>
      </a>
    {:else}
      <div></div>
    ...
    {#if adj.prev}
      <a href="/question/{adj.prev.id}" class="flex items-center gap-2 text-sm text-gray-500 hover:text-orange-600 transition-colors group text-right max-w-[45%]">
        <div>
          ...
          <p class="text-xs font-medium text-gray-700 group-hover:text-orange-600 line-clamp-2">{adj.prev.question.text.slice(0, 60)}…</p>
        </div>
        <svg class="w-4 h-4 flex-shrink-0 group-hover:translate-x-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
